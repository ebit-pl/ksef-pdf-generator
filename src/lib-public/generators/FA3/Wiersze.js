import { createHeader, createLabelTextArray, createSection, formatText, getContentTable, getTable, getTStawkaPodatku, getValue, } from '../../../shared/PDF-functions';
...
export function generateWiersze(faVat) {
    ...
    const faWiersze = getTable(faVat.FaWiersz).map((wiersz) => {
        const marza = addMarza(rodzajFaktury, isP_PMarzy, wiersz);
        if (getValue(wiersz.P_12)) {
            wiersz.P_12._text = getTStawkaPodatku(getValue(wiersz.P_12), 3);
        }
        return { ...wiersz, ...marza };
    });
    ...
    let content = getContentTable([...definedHeaderLp, ...definedHeader1, ...definedHeader2], faWiersze, 'auto');

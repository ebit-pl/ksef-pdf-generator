import { createHeader, createLabelText, createSection, createSubHeader, formatText, generateTwoColumns, getContentTable, getTable, } from '../../../shared/PDF-functions';
...
export function generateWarunkiTransakcji(warunkiTransakcji) {
    ...
    if (umowy.length > 0) {
        const tabUmowy = getContentTable(definedHeaderUmowy, umowy, '*', undefined, 20);
        if (tabUmowy.content) {
            Kolumny.umowy = [createSubHeader('Umowa'), tabUmowy.content];
        }
    }
    if (zamowienia.length > 0) {
        const tabZamowienia = getContentTable(definedHeaderZamowienia, zamowienia, '*', undefined, 20);

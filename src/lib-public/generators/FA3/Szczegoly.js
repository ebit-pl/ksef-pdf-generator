import { createHeader, createLabelText, createLabelTextArray, createSection, generateTwoColumns, getContentTable, getDifferentColumnsValue, getTable, getValue, hasColumnsValue, hasValue, } from '../../../shared/PDF-functions';
...
export function generateSzczegoly(faVat) {
    ...
    const tpLabel2 = [];
    const forColumns = [
        createLabelText('Data wystawienia, z zastrzeżeniem art. 106na ust. 1 ustawy: ', faVat.P_1, FormatTyp.Date),
...
function generateZaliczkaCzesciowa(zaliczkaCzesciowaData) {
    ...
    const tableZaliczkaCzesciowa = getContentTable(zaplataCzesciowaHeader, zaplataCzesciowa, 'auto');
...
function generateFakturaZaliczkowa(fakturaZaliczkowaData) {
    ...
    const fakturaZaliczkowa = getTable(fakturaZaliczkowaData);
    const fakturaZaliczkowaMapped = fakturaZaliczkowa.map(item => {
        const fp = ('NrFaZaliczkowej' in item && item.NrFaZaliczkowej) ? item.NrFaZaliczkowej : ('NrKSeFFaZaliczkowej' in item ? item.NrKSeFFaZaliczkowej : undefined);
        return {
            ...item,
    ...
    const tableFakturaZaliczkowa = getContentTable(fakturaZaliczkowaHeader, fakturaZaliczkowaMapped, 'auto', [0, 4, 0, 0]);

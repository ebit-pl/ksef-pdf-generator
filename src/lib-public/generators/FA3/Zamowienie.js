import { createHeader, createLabelTextArray, formatText, getContentTable, getTable, getTStawkaPodatku, getValue, } from '../../../shared/PDF-functions';
...
export function generateZamowienie(orderData, zamowienieKorekta, p_15, rodzajFaktury, KodWaluty) {
    ...
    const orderTable = getTable(orderData?.ZamowienieWiersz).map((el, index) => {
        ...
        if (getValue(el.P_12Z)) {
            el.P_12Z._text = getTStawkaPodatku(getValue(el.P_12Z), 3);
        }
        return el;
    });

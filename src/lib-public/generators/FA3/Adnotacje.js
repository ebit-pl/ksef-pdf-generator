import { createHeader, createLabelText, formatText, generateColumns, getTable, getValue, hasValue, verticalSpacing, } from '../../../shared/PDF-functions';
    ...
    if (adnotacje) {
        ...
        if (zwolnienie?.P_19?._text === '1') {
            ...
            if (zwolnienie.P_19A?._text) {
                addToColumn(firstColumn, secondColumn, createLabelText('Podstawa zwolnienia od podatku: ', 'Przepis ustawy albo aktu wydanego na podstawie ustawy, na podstawie którego podatnik stosuje zwolnienie od podatku'), true);
                addToColumn(firstColumn, secondColumn, createLabelText('Przepis ustawy albo aktu wydanego na podstawie ustawy: ', zwolnienie.P_19A._text), true);
            }
            if (zwolnienie.P_19B?._text) {
                ...
                addToColumn(firstColumn, secondColumn, createLabelText('Przepis dyrektywy: ', zwolnienie.P_19B._text), true);
            }
            if (zwolnienie.P_19C?._text) {
                ...
                addToColumn(firstColumn, secondColumn, createLabelText('Inna podstawa prawna: ', zwolnienie.P_19C._text), true);
            }
        }
...
export function generateDostawy(noweSrodkiTransportu) {
    ...
    if (nowySrodekTransportu?.length) {
        ...
        tableBody = nowySrodekTransportu.map((item) => {
            const value = [];
            const anyP22B = hasValue(item.P_22B) ||
            ...
            const anyP22N = hasValue(item.P_22B1) || hasValue(item.P_22B2) || hasValue(item.P_22B3) || hasValue(item.P_22B4);

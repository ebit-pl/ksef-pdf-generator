import { formatText, generateLine, getContentTable, getTable, getValue, hasValue, verticalSpacing, } from '../../../shared/PDF-functions';
import FormatTyp from '../../../shared/enums/common.enum';
import { DEFAULT_TABLE_LAYOUT } from '../../../shared/consts/const';
export function generateDokumnetUPO(potwierdzenie) {
    const dokumenty = getTable(potwierdzenie.Dokument);
    const result = [];
    const table = [];
    result.push(verticalSpacing(4));
    result.push(generateLine());
    result.push(verticalSpacing(8));
    result.push(formatText('Urzędowe poświadczenie odbioru dokumentu elektronicznego KSeF', FormatTyp.HeaderPosition));
    result.push(verticalSpacing(8));
    ...
    if (hasValue(potwierdzenie.NazwaStrukturyLogicznej)) {
        table.push([
            formatText('Nazwa pliku XSD struktury logicznej dotycząca przesłanego dokumentu:', FormatTyp.GrayBoldTitle),
    ...
    const tabDocument = getContentTable(definedHeader, documentData, 'auto');

import { createHeader, createLabelText, formatText, generateColumns, hasValue, verticalSpacing, } from '../../../shared/PDF-functions';
import FormatTyp from '../../../shared/enums/common.enum';
import { DEFAULT_TABLE_LAYOUT } from '../../../shared/consts/const';
export function generateAdnotacje(adnotacje) {
    const result = [];
    let firstColumn = [];
    const secondColumn = [];
    if (adnotacje) {
        if (adnotacje?.P_19?._text === '1') {
            ...
            if (adnotacje.P_19A?._text) {
                addToColumn(firstColumn, secondColumn, createLabelText('Podstawa zwolnienia od podatku: ', 'Przepis ustawy albo aktu wydanego na podstawie ustawy, na podstawie którego podatnik stosuje adnotacje od podatku'), true);
                addToColumn(firstColumn, secondColumn, createLabelText('Przepis ustawy albo aktu wydanego na podstawie ustawy: ', adnotacje.P_19A._text), true);
            }
            if (adnotacje.P_19B?._text) {
                addToColumn(firstColumn, secondColumn, createLabelText('Podstawa zwolnienia od podatku: ', 'Przepis dyrektywy 2006/112/WE, który zwalnia od podatku taką dostawę towarów lub takie świadczenie usług'), true);
                addToColumn(firstColumn, secondColumn, createLabelText('Przepis dyrektywy: ', adnotacje.P_19B._text), true);
            }
            if (adnotacje.P_19C?._text) {
                addToColumn(firstColumn, secondColumn, createLabelText('Podstawa zwolnienia od podatku: ', 'Inna podstawa prawna wskazującą na to, że dostawa towarów lub świadczenie usług korzysta ze zwolnienia'), true);
                addToColumn(firstColumn, secondColumn, createLabelText('Inna podstawa prawna: ', adnotacje.P_19C._text), true);
            }
        }
...
export function generateDostawy(adnotacje) {
    const result = [];
    const table = [];
    const anyP22B = hasValue(adnotacje.P_22B) ||
    ...
    const anyP22C = hasValue(adnotacje.P_22C) || hasValue(adnotacje.P_22C1);
    const anyP22D = hasValue(adnotacje.P_22D) || hasValue(adnotacje.P_22D1);
    ...
    if (anyP22B) {
        table.push([
            ...
            formatText('Dostawa dotyczy pojazdów lądowych, o których mowa w art. 2 pkt 10 lit. a ustawy', FormatTyp.Default),
        ]);
        ...
        table.push([
            formatText('Rodzaj pojazdu', FormatTyp.GrayBoldTitle),
            formatText('Dostawa dotyczy jednostek pływających, o których mowa w art. 2 pkt 10 lit. b ustawy', FormatTyp.Default),
        ]);
        ...
        table.push([
            formatText('Rodzaj pojazdu', FormatTyp.GrayBoldTitle),
            formatText('Dostawa dotyczy statków powietrznych, o których mowa w art. 2 pkt 10 lit. c ustawy', FormatTyp.Default),
        ]);

...
import { createHeader, createLabelText, createSection, createSubHeader, formatText, generateTwoColumns, getTable, hasValue, } from '../../../shared/PDF-functions';
import FormatTyp from '../../../shared/enums/common.enum';
import { getDateTimeWithoutSeconds, getOpisTransportuString, getRodzajTransportuString, } from '../../../shared/generators/common/functions';
import { generatePrzewoznik } from './Przewoznik';
export function generateTransport(transport, index) {
    const table = [];
    const columns = {
        transport: [],
        dane: [],
    ...
    if (transport.RodzajTransportu?._text) {
        columns.transport.push(createLabelText('Rodzaj transportu: ', getRodzajTransportuString(transport.RodzajTransportu)));
    }
        ...
        columns.transport.push(createLabelText('Opis innego rodzaju transportu: ', transport.OpisInnegoTransportu));
    }
    ...
    columns.dane.push(createLabelText('Data i godzina rozpoczęcia transportu: ', getDateTimeWithoutSeconds(transport.DataGodzRozpTransportu)));
    columns.dane.push(createLabelText('Data i godzina zakończenia transportu: ', getDateTimeWithoutSeconds(transport.DataGodzZakTransportu)));
    ...
    if (transport.WysylkaDo?.AdresL1) {
        columns.wysylkaDo.push(createSubHeader('Adres miejsca docelowego, do którego został zlecony transport', [0, 0, 0, 0]));
